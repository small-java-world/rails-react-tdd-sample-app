...
function BuildJscpdSection {
  ...
  [void]$sb.AppendLine()
  [void]$sb.AppendLine('推奨タイミング:')
  [void]$sb.AppendLine('- /tdd-green 完了直後に実行（新規実装による重複確認）')
  [void]$sb.AppendLine('- /tdd-refactor 開始前に実行（現状の重複を基準化）')
  [void]$sb.AppendLine('- /tdd-refactor 完了後に実行（重複削減の効果検証、HTML レポート確認）')
  [void]$sb.AppendLine()
